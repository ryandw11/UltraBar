...
public class BarCommand implements CommandExecutor {
    ...
    protected void processInfo(CommandSender s, Map<String, String> mp) {
        ...
        while (it.hasNext()) {
            ...
            } else if (pair.getKey().equalsIgnoreCase("countStyle") || pair.getKey().equalsIgnoreCase("cs")) {
                if (!s.hasPermission("ultrabar.bar.countstyle")) {
                    s.sendMessage(ChatColor.RED + "You do not have permission to set the count style.");
                    return;
                }
                CountStyle countStyle;
                if (pair.getValue().equalsIgnoreCase("up"))
                    countStyle = CountStyle.UP;
                else if (pair.getValue().equalsIgnoreCase("down"))
                    countStyle = CountStyle.DOWN;
                else {
                    s.sendMessage(ChatColor.RED + "Invalid count style value specified. Valid options: up, down");
                    return;
                }
